...
namespace BlazorGame.Client.Shared
{
    public class Level : IDisposable
    {
        ...
        private Task _loadingTiles;
        private Task _loadingTextures;
        ...
        public Level(ContentManager content, Stream fileStream, int levelIndex)
        {
            ...
            _loadingTiles = LoadTiles(fileStream);
        ...
        /// A stream containing the tile data.
        /// </param>
        private async Task LoadTiles(Stream fileStream)
        {
            ...
            for (int y = 0; y < Height; ++y)
            {
                for (int x = 0; x < Width; ++x)
                {
                    ...
                    tiles[x, y] = await LoadTile(tileType, x, y);
                }
            }
        ...
        private async Task<Tile> LoadTile(char tileType, int x, int y)
        {
            switch (tileType)
            {
                // Blank space
                ...
                // Floating platform
                case '-':
                    return await LoadTile("Platform", TileCollision.Platform);
                ...
                // Platform block
                case '~':
                    return await LoadVarietyTile("BlockB", 2, TileCollision.Platform);

                // Passable block
                case ':':
                    return await LoadVarietyTile("BlockB", 2, TileCollision.Passable);
                ...
                // Impassable block
                case '#':
                    return await LoadVarietyTile("BlockA", 7, TileCollision.Impassable);
        ...
        /// </param>
        private async Task<Tile> LoadVarietyTile(string baseName, int variationCount, TileCollision collision)
        {
            int index = random.Next(variationCount);
            return await LoadTile(baseName + index, collision);
        }
        ...
        {
            if (!_loadingTextures.IsCompleted || !_loadingTiles.IsCompleted) return;
